...
<body>
    ...
    <p style="margin-left: 50px;">This is to certify that <span style="text-transform: capitalize"><u>{{ $name }}</u></span> legal age, <span><u>{{ $data[0]->resident_info->age }}</u></span> Filipino Citizen
        and residing at <span><u>{{ $data[0]->resident_info->purok .' '. $data[0]->resident_info->street .' '. $data[0]->resident_info->block }}</u></span>
        <span>IV, Calamba City since <span><u>{{ $data[0]->resident_info->length_of_stay }}</u></span> up to present, is one of the indidents in our Barangay.</span>
    </p>
